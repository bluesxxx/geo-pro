...
> GEO PRO — открытая система контент-инжиниринга GEO (Generative Engine Optimization) и мультисайтовой дистрибуции. Она объединяет базы знаний, библиотеки материалов, промпты, AI-задачи, ревью и публикацию, аналитику, пакеты целевых сайтов GEO PRO Agent, каналы WordPress REST, универсальные HTTP API-каналы и удалённую статическую публикацию в единый управляемый поток, который превращает достоверные материалы в публикуемые, отслеживаемые и распределяемые GEO-активы.
...
| 📡 Мультисайтовая дистрибуция | Каналы GEO PRO Agent, WordPress REST и универсального HTTP API, секреты, пакеты целевых сайтов, статический режим, rewrite-правила, удалённое редактирование/удаление, очереди и логи |
...
| 🌍 i18n админки | Китайский, английский, японский, испанский, русский и португальский (Бразилия), включая модули GEO PRO 2.0 |
...
Ключевые изменения GEO PRO 2.0:
...
- **Дистрибуция работает end-to-end**: каналы GEO PRO Agent, WordPress REST и универсального HTTP API, секреты, проверка соединения, пакеты целевых сайтов, статический/rewrite режимы, синхронизация настроек, очереди, логи, удалённое редактирование и удаление.
- **Область публикации явная**: задача может публиковать локально и в каналы, только в каналы или только в локальный GEO PRO; локальный режим отключает выбор каналов.
...
- **Локализация покрывает текущие ключи админки**: модули GEO PRO 2.0 больше не показывают raw translation keys или английский fallback.
...
GEO PRO подходит для таких практических сценариев:
...
- **Внутренняя GEO CMS / операционная система контента**
  Для использования GEO PRO как внутреннего backend-инструмента для моделей, материалов, знаний, промптов, ревью и публикации. Цель — повысить эффективность команды.
...
GEO PRO не предназначен для создания ложного контента или массового информационного шума. Его задача — повышать эффективность AI-маркетинга и GEO-операций за счет работы с доверенным контентом.
...
- **Как GEO-подканал существующего сайта**
  Используйте GEO PRO в поддомене, каталоге или отдельном канале без полной переделки основного сайта.
...
Если база знаний слабая, автоматизация лишь масштабирует шум. В GEO PRO **качество базы знаний должно быть на первом месте**.
...
GEO PRO распространяется по [Apache License 2.0](../../LICENSE). Лицензия разрешает личное и коммерческое использование, изменение, распространение и приватное развертывание при соблюдении уведомлений о лицензии, авторских правах, изменениях, патентных условиях и отказе от гарантий.
