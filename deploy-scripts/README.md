# GEO PRO 部署脚本 / Deployment Scripts

这个目录用于存放 GEO PRO 的参考部署脚本，方便技术人员在常见云服务器、VPS、Docker 主机或面板服务器上快速完成环境自检和生产部署。
...
| 变量 | 默认值 | 说明 |
| --- | --- | --- |
| `GEOFLOW_REPO_URL` | `https://github.com/bluesxxx/geo-pro.git` | 源码仓库地址 |
...
这个动作只会删除当前执行的脚本文件，不会删除已部署的 GEO PRO 源码目录。
...
This folder contains reference scripts for technical operators who want a faster, repeatable GEO PRO deployment path.
...
| --- | --- | --- |
| `GEOFLOW_REPO_URL` | `https://github.com/bluesxxx/geo-pro.git` | Source repository URL |
...
This only removes the executed script file. It does not remove the deployed GEO PRO source code.
